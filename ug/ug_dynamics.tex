...
\subsubsection{Monte Carlo barostat}



\NAMD\ provides constant pressure simulation using Monte Carlo method to control
volume fluctuations in the barostat. This method should be combined with a method
of temperature control, in order to simulate the NPT ensemble. This feature is only supported in \emph{GPU Resident mode}. 

Please note that before switching to constant pressure simulations, you will need to
equilibrate your system using NVT ensemble simulation.

In this method, a trial volume-change, $\Delta V$, is generated uniformly,
within the range of $[-\Delta V_{\rm max}, +\Delta V_{\rm max}]$, where $\Delta V_{\rm max}$
is the maximum volume-change, adjusted to obtain the target acceptance ratio.

The probability of accepting volume-change from $V$ to $V^{'} = V + \Delta V$ is:

\begin{eqnarray*}
       P^{acc} &=& {\rm min} \left[1, \left(\frac{V^{'}}{V}\right)^{N} e^{-\beta \left(\Delta U + P\Delta V - \gamma \Delta A\right)} \right] \\
       \Delta U &=& U\left(\textbf{s}^{N}, V^{'}\right) - U\left(\textbf{s}^{N}, V\right) \\
       \Delta V &=& V^{'} - V \\
       \Delta A &=& {\textrm{change in x-y area}}  \\
       \beta &=& \frac{1}{k T} \\
       \textbf{s}^{N} &=& {\textrm{reduced coordinates}}
\end{eqnarray*}
where, $\gamma$, $P$, $T$, $U\left(\textbf{s}^{N}, V^{'}\right)$, and $U\left(\textbf{s}^{N}, V\right)$
are the target surface tension, target pressure, system temperature, and total potential energy of the system
at \textit{new} and \textit{old} configuration, respectively. $N$ denotes the total number of
...
\item
\NAMDCONFWDEF{MonteCarloPressure}{use Monte Carlo pressure control?}{{\tt on} or {\tt off}}{{\tt off}}
{Specifies whether or not Monte Carlo pressure control is active.
If set to {\tt on}, then the parameters {\tt MonteCarloPressureTarget} and
 {\tt MonteCarloTemp} must be set. The parameters {\tt MonteCarloPressureFreq},
 {\tt MonteCarloMaxVolume}, {\tt MonteCarloAcceptanceRate}, {\tt MonteCarloAdjustmentFreq},
 and {\tt SurfaceTensionTarget} can optionally be set to control the behavior of this feature.}

\item
\NAMDCONF{MonteCarloPressureTarget}{target pressure (bar)}{positive decimal}
{Specifies target pressure for Monte Carlo barostat.
A typical value would be 1.01325 bar, atmospheric pressure at sea level.}

\item
\NAMDCONF{MonteCarloTemp}{system temperature (K)}{positive decimal}
{Specifies temperature for Monte Carlo barostat. This should be set equal to
the target temperature for the chosen method of temperature control.}

\item
\NAMDCONFWDEF{MonteCarloPressureFreq}{how often attempt to change volume?}{positive multiple of
{\tt nonbondedFrequency} and {\tt fullElectFrequency}}{50 * {\tt nonbondedFrequency}
or 50 * {\tt fullElectFrequency} if used}
{Specifies number of timesteps between attempt to change the volume and rescale
the coordinates. A typical value for this parameter could be 50.}

\item
\NAMDCONFWDEF{MonteCarloMaxVolume}{initial maximum volume-change for each axis dimension (\AA$^3$) }
{vector of positive decimal and less than $0.3V$}{$0.01V$ $0.01V$ $0.01V$}
{Optionally specifies the initial maximum volume-change for each axis dimension 
($\Delta V_{\rm max}$) in Monte Carlo pressure control.
If {\tt extendedSystem} is also specified, value is overridden.
There is typically no reason to set this parameter.}

\item
\NAMDCONFWDEF{MonteCarloAcceptanceRate}{target volume-change acceptance rate}{decimal between 0.2 and 0.8}{0.5}
{Specifies the desired acceptance rate for volume-change attempt. The $\Delta V_{\rm max}$
is dynamically adjusted to reach the target acceptance rate. Higher acceptance rate
results in small volume fluctuations, while lower acceptance rate results in
large volume fluctuations.}

\item
\NAMDCONFWDEF{MonteCarloAdjustmentFreq}{how often adjust maximum volume-change?}
{positive integer}{30}
{Specifies the number of volume-change attempts, to collect acceptance
rate statistics, in order to adjust maximum volume-change ($\Delta V_{\rm max}$).
Higher value would provide more accurate statistics, but might
require longer simulation to reach equilibration. A typical value of 30
for isotropic and 50 for anisotropic cell fluctuation could be
...
{decimal}{0.0}{Specifies surface tension target.  Must be used with
{\tt useFlexibleCell} and periodic boundary conditions.  The pressure
specified in {\tt MonteCarloPressureTarget} becomes the pressure along the z
axis, and surface tension is applied in the x-y plane.}

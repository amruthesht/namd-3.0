 ...
 *      $Revision: 1.44 $       $Date: 2024/03/01 02:17:44 $
 *
 ***************************************************************************/
...
static int read_pdb_record(FILE *f, char *retStr) {
  ...
  if (inbuf != fgets(inbuf, PDB_RECORD_LENGTH + 2, f)) {
    ...
    if (!strncmp(inbuf, "ATOM ",  5) || !strncmp(inbuf, "HETATM", 6)) {
      ...
      recType = PDB_HEADER;
    } else if (!strncmp(inbuf, "ENDBRANCH", 9)) {  /* PDBQT variant used by AutoDock */
      recType = PDB_UNKNOWN;
    } else if (!strncmp(inbuf, "ENDROOT", 7)) {    /* PDBQT variant used by AutoDock */
      recType = PDB_UNKNOWN;

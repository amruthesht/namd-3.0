  ...
  public:
    ...
    void unregisterLcpoTypePickup(Compute *cid, Box<Patch,int>**const box);
     ...
     // DMK - Atom Separation (water vs. non-water)
       ...
       int getNumWaterAtoms() const { return numWaterAtoms; }
     #endif

     int getNumFixedAtoms() const { return numFixedAtoms; }  // not updated
     void setNumFixedAtoms(int numFixed) { numFixedAtoms=numFixed; }  // not updated
     PatchID getPatchID() const { return patchID; }
     int getNumComputes() const { return positionComputeList.size(); }

     CompAtomExt* getCompAtomExtInfo() { return pExt.begin(); }
     CompAtom* getCompAtomInfo() {return p.begin(); }
#ifdef NAMD_KNL
     ...
     // DMK - NOTE : Just placing the variables in public for now so only one location, move to protected if this actually helps performance
       ...
       int nbond_compute_position;
       int bond_compute_position;
       int slow_compute_position;
#endif

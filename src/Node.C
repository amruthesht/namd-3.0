...
void Node::startup() {
  NAMD_EVENT_START(1, NamdProfileEvent::NAMD_STARTUP);
...
void Node::buildSequencers() {
  ...
  if ( ! CkMyPe() ) {
    ...
    // OK so I need to wait out here if I want to invoke c_out cm
    pdata->c_out = controller;
#endif
  }
...
void Node::run()
{
//  NAMD_EVENT_START(1, NamdProfileEvent::NODE_RUN);
...
//  NAMD_EVENT_STOP(1, NamdProfileEvent::NODE_RUN);
}

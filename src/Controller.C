...
void Controller::printStep(int step, NodeReduction *nr){
  char traceNote[24];
  this->nodeReduction = nr;
  const int numberOfSteps = simParams->N;
  const int stepsPerCycle = simParams->stepsPerCycle;
  const int zeroMomentum = simParams->zeroMomentum;
  nbondFreq = simParams->nonbondedFrequency;
  ...
  if (dofull)
    ...
    slowFreq = simParams->nonbondedFrequency;
  if ( step >= numberOfSteps ) slowFreq = nbondFreq = 1;
  // Ok, now I need a way to set reduction to the nodeReduction
...
void Controller::calcPressure(int step, int minimize,
  ...
  if ( (volume=lattice.volume()) != 0. )
  {
    ...
    if(simParams->CUDASOAintegrate && step != 0){
...
void Controller::computeTIderivative() {
    // Note: WCA is incompatible with TI
    BigReal l1 = simParams->alchLambda;
    BigReal l2 = 1. - simParams->alchLambda;
...
#ifdef NODEGROUP_FORCE_REGISTER
Tensor Controller::getPositionRescaleFactor(int step){
  Tensor t = publishedRescaleFactors[step];
  publishedRescaleFactors.erase(step);
  return t;
}
int Controller::getMCAcceptance(int step){
  int accepted = publishedMCAcceptance[step];
  publishedMCAcceptance.erase(step);
  return accepted;
}
#endif

...
//these will return void for now
ReductionValue::ReductionValue(){ 
  reducedValue = 0.0;
  valueLock = CmiCreateLock();
} 

ReductionValue::~ReductionValue(){
  CmiDestroyLock(this->valueLock);
}

// atomic updates to reducedValue
double ReductionValue::operator+=(double rvalue){
  //reducedValue.fetch_add(rvalue, std::memory_order_relaxed);
  CmiLock(valueLock);
  reducedValue += rvalue;
  CmiUnlock(valueLock);
  return reducedValue;
...
double ReductionValue::operator+=(int rvalue){
  //reducedValue.fetch_add(rvalue, std::memory_order_relaxed);
  CmiLock(valueLock);
  reducedValue += rvalue;
  CmiUnlock(valueLock);
...
double ReductionValue::operator+=(unsigned int rvalue){
  //reducedValue.fetch_add(rvalue, std::memory_order_relaxed);
  CmiLock(valueLock);
  reducedValue += rvalue;
  CmiUnlock(valueLock);
...
NodeReduction::NodeReduction(){
  //memset(set, 0, sizeof(ReductionValue)*REDUCTION_MAX_RESERVED);
  for(int i = 0; i < REDUCTION_MAX_RESERVED; i++){
    this->set[i].reducedValue = 0.0;
  }
...
void NodeReduction::zero(){
  // this memset is going to eff everything up i think
  //memset(set, 0, sizeof(ReductionValue)*REDUCTION_MAX_RESERVED);
#pragma unroll
  for(int i = 0; i < REDUCTION_MAX_RESERVED; i++){
    this->set[i].reducedValue = 0.0;
...
#pragma unroll
  for(int i = 0; i < REDUCTION_MAX_RESERVED; i++){
    this->set[i].reducedValue = other->set[i].reducedValue;
  }
}

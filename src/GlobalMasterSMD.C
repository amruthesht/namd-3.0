...
void GlobalMasterSMD::calculate() {
  ...
  Position diffCOM = curcm - cm;
  BigReal diff = diffCOM*moveDir;
  BigReal preFactor = moveVel*currentTime - diff; 
  ...
  Force f = k*preFactor*moveDir + k2*(diff*moveDir - diffCOM);
  modifyGroupForces().resize(1);
  modifyGroupForces()[0] = f;
  // calculate energy
  // energy for restraint along the direction
  BigReal energy = 0.5*k*preFactor*preFactor;
  energy += 0.5*k2*(diffCOM.length2() - diff*diff); 

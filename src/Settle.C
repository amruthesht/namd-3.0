...
static inline BigReal det_3by3(BigReal A[4][4])
{
    return
...
void MSHAKEIterate(const int icnt, const RattleParam* rattleParam,
  const BigReal *refx, const BigReal *refy, const BigReal *refz,
  BigReal *posx, BigReal *posy, BigReal *posz,
  const BigReal tol2, const int maxiter,
  bool& done, bool& consFailure)
...
#endif
void LINCS(const int icnt, const RattleParam* rattleParam,
  const BigReal *refx, const BigReal *refy, const BigReal *refz,
  BigReal *posx, BigReal *posy, BigReal *posz,
  const BigReal tol2, const int maxiter,
  bool& done, bool& consFailure)
  ...
  bool& done, bool& consFailure)
{
    ...
    //VectorXr pabx(icnt), rabx(icnt);
    //VectorXr paby(icnt), raby(icnt);
    //VectorXr pabz(icnt), rabz(icnt);
    BigReal* pabx = new BigReal[icnt];
    BigReal* rabx = new BigReal[icnt];
    BigReal* paby = new BigReal[icnt];
    BigReal* raby = new BigReal[icnt];
    BigReal* pabz = new BigReal[icnt];
    BigReal* rabz = new BigReal[icnt];
    ...
    for(loop = 0; loop < maxiter; ++loop)
    {
        if(!done)
        {
            ...
            {
                int idx2 = i / icnt;
                int idx1 = i - icnt * idx2;
                BigReal rma = rattleParam[idx1].rma;
                A(idx1,idx2) = 2.*(pabx[idx1]*rabx[idx2]+paby[idx1]*raby[idx2]+pabz[idx1]*rabz[idx2])*rma;
            }
            for(int i = 0; i < icnt; ++i)
            {
                //BigReal rmb = rattleParam[i].rmb;
                //A(i,i) += 2.*(pabx[i]*rabx[i]+paby[i]*raby[i]+pabz[i]*rabz[i])*rmb;
                A(i,i) += A(i,i)*rattleParam[i].rmb/rattleParam[i].rma;
            }
            //sigma = A.transpose()*sigma;
            //A = A.transpose() * A;
            //PartialPivLU<MatrixXr> lu;
            //lu.compute(A);
            lambda = A.partialPivLu().solve(sigma);
            for(int i = 0; i < icnt; ++i)
            {
                int a = rattleParam[i].ia;
                int b = rattleParam[i].ib;
    ...
    if(loop == maxiter)
        consFailure = true;
    //write out the total number of iteration number
    //std::fstream ofile;
    //ofile.open("iteration_loops_mshake", std::fstream::app);
    //ofile << loop << std::endl;
    //ofile.close();
  ...
  bool& done, bool& consFailure)
{
    ...
    if(!done)
    {
        ...
        LLT<MatrixXr> lltOfS(S);
        VectorXr lambda = lltOfS.solve(r_unc);
        ...
        //write out the total number of iteration number
        //std::fstream ofile;
        //ofile.open("iteration_loops_lincs", std::fstream::app);
        //ofile << iter << std::endl;
        //ofile.close();
    }

...
void MSHAKE_CUDA(int*, const int size, const RattleParam* rattleParam,
  BigReal *refx, BigReal *refy, BigReal *refz,
...
{ 
  ...
  if (simParams->SOAintegrateOn) {
    PatchDataSOA_initialize( &patchDataSOA );
    sort_solvent_atoms();
    copy_atoms_to_SOA();
#if 0
  ...
  if (simParams->watmodel == WaterModel::TIP4) init_tip4();
  else if (simParams->watmodel == WaterModel::SWM4) init_swm4();
...
// begin SOA
//
void HomePatch::positionsReady_SOA(int doMigration)
{    
  SimParameters *simParams = Node::Object()->simParameters;
  // char prbuf[32];
...
////////////////////////////////////////////////////////////////////////
//
// GPU migration code path
//

#ifdef NODEGROUP_FORCE_REGISTER

void HomePatch::updateAtomCount(const int n, const int reallocate) {
  numAtoms = n;
  // See MigrationCUDAKernel.h
  if (n > MigrationCUDAKernel::kMaxAtomsPerPatch) {
    NAMD_die("Device migration does not currently support patches with greater than 2048 atoms.\n"
      "Please run with a smaller margin or without device migration.\n");
  }
}

void HomePatch::updateAtomBuffers() {
  atom.resize(numAtoms);
  cudaAtomList.resize(numAtoms);
  p.resize(numAtoms);
  pExt.resize(numAtoms);

  size_t nbytes = patchDataSOA.numBytes;
  if (nbytes != PatchDataSOA_set_size(&patchDataSOA, numAtoms, 2048)) {
#if defined(NAMD_CUDA) || defined(NAMD_HIP)
    reallocate_host<unsigned char>(&soa_buffer,&soa_buffer_size, PatchDataSOA_set_size(&patchDataSOA, numAtoms));
    PatchDataSOA_set_buffer( &patchDataSOA, soa_buffer);
#else
    soa_buffer.resize( PatchDataSOA_set_size(&patchDataSOA, numAtoms) );
    PatchDataSOA_set_buffer( &patchDataSOA, soa_buffer.begin() );
#endif
  }
}

void HomePatch::clearAtomMap() {
  atomMapper->unregisterIDsFullAtom(atom.begin(),atom.end());
}

void HomePatch::positionsReady_GPU(int doMigration, int startup) {
  SimParameters *simParams = Node::Object()->simParameters;
  if (doMigration) {
    // make sure to invalidate RATTLE lists when atoms move
    rattleListValid_SOA = false;
    rattleListValid = false;
    //invalidate gridforced list
    gridForceIdxChecked=false;

  }
  if (startup) {
    const int numAtomAlloc = 2048;
    atom.reserve(numAtomAlloc);
    cudaAtomList.reserve(numAtomAlloc);
    p.reserve(numAtomAlloc);
    pExt.reserve(numAtomAlloc);

    for ( int j = 0; j < Results::maxNumForces; ++j ) {
      f[j].reserve(numAtomAlloc);
    }
  }
  ...
  if (nbytes != PatchDataSOA_set_size(&patchDataSOA, numAtoms, 2048)) {
#if defined(NAMD_CUDA) || defined(NAMD_HIP)
    reallocate_host<unsigned char>(&soa_buffer,&soa_buffer_size, PatchDataSOA_set_size(&patchDataSOA, numAtoms, 2048));
    PatchDataSOA_set_buffer( &patchDataSOA, soa_buffer);
#else
    soa_buffer.resize( PatchDataSOA_set_size(&patchDataSOA, numAtoms, 2048) );
    PatchDataSOA_set_buffer( &patchDataSOA, soa_buffer.begin() );
#endif
  }
  ...
  // This is set during startup depending on which features are enabled.
  const bool updateAtomMap = simParams->updateAtomMap;

  // If startup or update atom map, then we need to update some data structures. 
  // For startup this is mainly for tuple generations
  if (startup || updateAtomMap) {
    // These are used in the atom map, so are needed for startup and updateAtomMap
    CompAtomExt * __restrict pExt_i = pExt.begin();
#ifndef USE_NO_BITFIELDS
    CompAtomExtCopy *pExtCopy_i = (CompAtomExtCopy *) pExt_i;
#endif
    for (int i=0;  i < numAtoms;  i++) {
      const uint32 atomFixed = uint32(atom[i].atomFixed);
      const uint32 groupFixed = uint32(atom[i].groupFixed);
      const uint32 id = uint32(atom[i].id);
#ifndef USE_NO_BITFIELDS
      ASSERT(atomFixed == 0 || atomFixed == 1);
      ASSERT(groupFixed == 0 || groupFixed == 1);
      uint32 atomFixedBit = NAMD_ATOM_FIXED_MASK * atomFixed;
      uint32 groupFixedBit = NAMD_GROUP_FIXED_MASK * groupFixed;
      pExtCopy_i[i].id = (id | atomFixedBit | groupFixedBit);
#else
      pExt_i[i].id = id;
      pExt_i[i].atomFixed = atomFixed;
      pExt_i[i].groupFixed = groupFixed;
#endif // USE_NO_BITFIELDS
    }
  } 
  if (startup) {
    // These are used in the exclusion tuple generation, so are only needed for startup
    CompAtom * __restrict p_i = p.begin();
    for (int i=0;  i < numAtoms;  i++) {
      p_i[i].vdwType = atom[i].vdwType;
    }
    if (simParams->alchOn) {
      for (int i=0;  i < numAtoms;  i++) {
        p_i[i].partition = atom[i].partition;
      }
    }
    // copy data from AOS into SOA
    for (int i=0;  i < numAtoms;  i++) {
      patchDataSOA.id[i] = int(atom[i].id);
      patchDataSOA.atomFixed[i] = int(atom[i].atomFixed);
      patchDataSOA.groupFixed[i] = int(atom[i].groupFixed);
    }
  } else if (updateAtomMap) {
    // This might be overkill for updating atom map
    copy_atoms_to_SOA();
  }
...
// DMK
  ...
  Patch::positionsReady(doMigration, startup);
...
//
// end of GPU migration code path
//
////////////////////////////////////////////////////////////////////////
...
void HomePatch::sort_solvent_atoms() {
#if defined(NAMD_NVTX_ENABLED) || defined(NAMD_CMK_TRACE_ENABLED)  || defined(NAMD_ROCTX_ENABLED)
  char ssabuf[32];
  sprintf(ssabuf, "%s: %d", NamdProfileEventStr[NamdProfileEvent::SORT_SOLVENT_ATOMS], this->getPatchID());
  NAMD_EVENT_START_EX(1, NamdProfileEvent::SORT_SOLVENT_ATOMS, ssabuf);
#endif


  // goal is to move SETTLE water molecules to end of FullAtom array
  // first we need to count the number of SETTLE water molecules
  sortatom.resize(numAtoms);
  FullAtom * __restrict a = atom.begin();
  FullAtom * __restrict sa = sortatom.begin();
  numSolventAtoms = 0;
  numSoluteAtoms = 0;
  int i, j, k;
  for (i = 0;  i < numAtoms;  i++) {
    if (a[i].isWater) numSolventAtoms++;
  }
  j = 0;  // starting index for solute
  numSoluteAtoms = numAtoms - numSolventAtoms;
  k = numSoluteAtoms;  // starting index for water
  for (i = 0;  i < numAtoms;  i++) {
    if (a[i].isWater) {
      sa[k++] = a[i];
    }
  ...
  // replace atom array by sorted array by swapping memory buffers
  atom.swap(sortatom);
  // XXX improve next line to avoid division
  // XXX should NOT assume 3 atoms per water
  numWaters = numSolventAtoms / 3;

  NAMD_EVENT_STOP(1, NamdProfileEvent::SORT_SOLVENT_ATOMS);
...
size_t PatchDataSOA_set_size(
  ...
  if (natoms > p->maxAtoms) {
    ...
    int numdoubles = (simParams->colvarsOn || simParams->tclForcesOn) ? 34: 31; 
...
void HomePatch::copy_atoms_to_SOA() {
  ...
  // do not resize buffer and reset internal array pointers
  // unless the buffer size changes
  size_t nbytes = patchDataSOA.numBytes;
  if (nbytes != PatchDataSOA_set_size(&patchDataSOA, numAtoms)) {
#if defined(NAMD_CUDA) || defined(NAMD_HIP)
    reallocate_host<unsigned char>(&soa_buffer,&soa_buffer_size, PatchDataSOA_set_size(&patchDataSOA, numAtoms));
    PatchDataSOA_set_buffer( &patchDataSOA, soa_buffer);
#else
  ...
  for (int i=0;  i < numAtoms;  i++) {
    ...
    patchDataSOA.charge[i] = atom[i].charge;
    patchDataSOA.vdwType[i] = int(atom[i].vdwType);
    patchDataSOA.partition[i] = int(atom[i].partition);
    // XXX nonbondedGroupSize is always recalculated
    //patchDataSOA.nonbondedGroupSize[i] = int(atom[i].nonbondedGroupSize);
    patchDataSOA.hydrogenGroupSize[i] = int(atom[i].hydrogenGroupSize);
    patchDataSOA.isWater[i] = int(atom[i].isWater);
    // XXX sortOrder is recalculated
    // XXX and defined only for NAMD_CUDA or NAMD_MIC
    //patchDataSOA.sortOrder[i] = int(atom[i].sortOrder);
    patchDataSOA.id[i] = int(atom[i].id);
#ifdef MEM_OPT_VERSION
    patchDataSOA.exclId[i] = int(atom[i].exclId);
    patchDataSOA.sigId[i] = int(atom[i].sigId);
#endif
    patchDataSOA.atomFixed[i] = int(atom[i].atomFixed);
    patchDataSOA.groupFixed[i] = int(atom[i].groupFixed);
    ...
    patchDataSOA.fixedPosition_x[i] = atom[i].fixedPosition.x;
    patchDataSOA.fixedPosition_y[i] = atom[i].fixedPosition.y;
    patchDataSOA.fixedPosition_z[i] = atom[i].fixedPosition.z;
    patchDataSOA.mass[i] = atom[i].mass;
    patchDataSOA.langevinParam[i] = atom[i].langevinParam;
    patchDataSOA.status[i] = atom[i].status;
    patchDataSOA.transform_i[i] = atom[i].transform.i;
    patchDataSOA.transform_j[i] = atom[i].transform.j;
    patchDataSOA.transform_k[i] = atom[i].transform.k;
    patchDataSOA.migrationGroupSize[i] = atom[i].migrationGroupSize;
...
void HomePatch::copy_forces_to_SOA() {
  ...
  const int saveForce = (simParams->tclForcesOn || simParams->colvarsOn);
  ...
  if (flags.doNonbonded) {
    ...
    if(saveForce) {
      for (int i=0;  i < numAtoms;  i++) {
        Force f = fnbond[i];
        patchDataSOA.f_nbond_x[i] = f.x;
        patchDataSOA.f_nbond_y[i] = f.y;
        patchDataSOA.f_nbond_z[i] = f.z;
        patchDataSOA.f_saved_nbond_x[i] = f.x;
        patchDataSOA.f_saved_nbond_y[i] = f.y;
        patchDataSOA.f_saved_nbond_z[i] = f.z;
      }
    } else {
      for (int i=0;  i < numAtoms;  i++) {
    ...
    // we dont set the saved force to zero if we dont have nonbonded force.
  }
  if (flags.doFullElectrostatics) {
    ...
    if(saveForce) {
      for (int i = 0;  i < numAtoms;  i++) {
        Force f = fslow[i];
        patchDataSOA.f_slow_x[i] = f.x;
        patchDataSOA.f_slow_y[i] = f.y;
        patchDataSOA.f_slow_z[i] = f.z;
        patchDataSOA.f_saved_slow_x[i] = f.x;
        patchDataSOA.f_saved_slow_y[i] = f.y;
        patchDataSOA.f_saved_slow_z[i] = f.z;
      }
    } else {
      for (int i = 0;  i < numAtoms;  i++) {
    ...
    // we dont set the saved force to zero if we dont have slow force. 
  }
  NAMD_EVENT_STOP(1, NamdProfileEvent::COPY_FORCES_TO_SOA);
}
...
void HomePatch::zero_global_forces_SOA() {
  memset(this->patchDataSOA.f_global_x, 0, sizeof(double)*numAtoms);
  memset(this->patchDataSOA.f_global_y, 0, sizeof(double)*numAtoms);
  memset(this->patchDataSOA.f_global_z, 0, sizeof(double)*numAtoms);
}
...
//
// XXX TODO: This operation could be moved to the gpu (?)
...
void HomePatch::separateAtoms() {
  ...
  int wathgsize = 3;
  if (simParams->watmodel == WaterModel::TIP4) wathgsize = 4;
  else if (simParams->watmodel == WaterModel::SWM4) wathgsize = 5;

...
void Molecule::read_psf_file(char *fname, Parameters *params)
{
  ...
  if (numLphosts == 0) {
    ...
    is_lonepairs_psf = 0;
  } else if (simParams->CUDASOAintegrate) {
    NAMD_die("The GPU-resident mode does not support systems with lone pairs except 4-site water models.");
  }
...
void Molecule::process_bonds(Parameters *params) {
  ...
  /* Haochuan: NAMD should not die immediately if a bond is not found,
   * because the bond could involve a lone pair and appear both in
   * the NUMLP and NBOND section.
   */
  ...
  for (int old_read=0; old_read < origNumBonds; ++old_read)
  {
    ...
    *b = bonds[old_read];
    ...
    /* Haochuan: NAMD should raise an error if (i) the bond parameter not
     * found, AND (ii) this bond does not involve lone pair.
     */
    if (!is_lp_bond && !bond_found) {
      char err_msg[512];
      snprintf(err_msg, sizeof(err_msg),
          "UNABLE TO FIND BOND PARAMETERS FOR %s %s (ATOMS %i %i)",
           atomNames[atom_nums[0]].atomtype, atomNames[atom_nums[1]].atomtype,
           b->atom1+1, b->atom2+1);
      NAMD_die(err_msg);
    }
...
// To sort the 2d vector based on column size
...
void Molecule::build_atom_status(void) {
  ...
  if (simParams->watmodel == WaterModel::TIP4 || is_lonepairs_psf) {
  ...
  if (simParams->watmodel == WaterModel::SWM4) {
    for (i = 0;  i < numAtoms;  i++) {
  ...
  if ( simParams->rigidBonds != RIGID_NONE || simParams->mollyOn ) {
    ...
    if ( (simParams->watmodel == WaterModel::TIP4 && numLonepairs > 0)
        || (simParams->watmodel == WaterModel::SWM4 && numDrudeWaters > 0)) {
...
void Molecule::compute_LJcorrection() {
  ...
  */
  /*
   * Haochuan: Account for extra LJ types in solute scaling
   */
  const Bool soluteScalingOn = simParams->soluteScalingOn;
  const int ss_dim = soluteScalingOn ? ss_num_vdw_params : 0;
  const int table_dim_org = params->get_num_vdw_params();
  const int LJtypecount = params->get_num_vdw_params() + ss_dim;
  /*
   * Haochuan: Check if ss_vdw_type is NULL. If true, then
   * compute_LJcorrection may be called after build_ss_flags,
   * which should definitely a bug.
   */
  if (soluteScalingOn && (ss_vdw_type == NULL)) {
    NAMD_bug("Solute scaling is used but ss_vdw_type is NULL. "
             "It is likely that compute_LJcorrection() is called "
             "before build_ss_flags().");
  }
  Real A, B, A14, B14;
  ...
  for (int i = 0; i < LJtypecount; i++) {
    for (int j = 0; j < LJtypecount; j++) {
      const int i_type = soluteScalingOn ? ((i >= table_dim_org) ? ss_vdw_type[i-table_dim_org] : i) : i;
      const int j_type = soluteScalingOn ? ((j >= table_dim_org) ? ss_vdw_type[j-table_dim_org] : j) : j;
      if (params->get_vdw_pair_params(i_type, j_type, &A, &B, &A14, &B14)) {
        ATable[i*LJtypecount + j] = A;
        BTable[i*LJtypecount + j] = B;
      }
      else {
        params->get_vdw_params(&sigma_i,&epsilon_i,&sigma_i14,&epsilon_i14,i_type);
        params->get_vdw_params(&sigma_j,&epsilon_j,&sigma_j14,&epsilon_j14,j_type);
        BigReal sigma_ij =
...
/****************************************************************************/
/*  FUNCTION compute_LJcorrection_alternative                               */
/*                                                                          */
/*  Compute the energy and virial tail corrections to the Lennard-Jones     */
/*  potential. The approximation used for heterogenous systems is to compute*/
/*  all interaction energy, including atom types with no VDW interaction and*/
/*  self-self interactions.                                                 */
/*                                                                          */
/*  REFERENCES                                                              */
/*   There is no reference for this method, because the original function   */
/*   was derived for pure atom type. However, all MC codes use this method */
/****************************************************************************/
void Molecule::compute_LJcorrection_alternative() {
  ...
  */
  ...
  /*
   * Haochuan: Check if ss_vdw_type is NULL. If true, then
   * compute_LJcorrection_alternative may be called after build_ss_flags,
   * which should definitely a bug.
   */
  if (soluteScalingOn && (ss_vdw_type == NULL)) {
    NAMD_bug("Solute scaling is used but ss_vdw_type is NULL. "
             "It is likely that compute_LJcorrection_alternative() is called "
             "before build_ss_flags().");
  }
  Real A, B, A14, B14;
  Real sigma_i, sigma_i14, epsilon_i, epsilon_i14;
  ...
  for (int i=0; i < LJtypecount; i++) {
    for (int j=0; j < LJtypecount; j++) {
      A = ATable[i*LJtypecount + j];
      B = BTable[i*LJtypecount + j];
      npairs = BigReal(numAtomsByLjType[i])*BigReal(numAtomsByLjType[j]);
      sumOfAs += npairs*A;
      sumOfBs += npairs*B;
    }
  }
  ...
  */
  if (simParams->alchOn && simParams->alchVdwLambdaEnd > 0.0) {
    ...
    if ( ! CkMyPe() ) {
      BigReal inv_sizeSq = 1.0 / (numAtoms * numAtoms);
      iout << iINFO << "LONG-RANGE LJ: APPLYING ANALYTICAL CORRECTIONS TO "
           << "ENERGY AND PRESSURE\n" << endi;
      iout << iINFO << "LONG-RANGE LJ: AVERAGE A0 AND B0 COEFFICIENTS "
           << LJAvgA2*inv_sizeSq << " AND " << LJAvgB2*inv_sizeSq << "\n" << endi;
      iout << iINFO << "LONG-RANGE LJ: AVERAGE A1 AND B1 COEFFICIENTS "
           << LJAvgA1*inv_sizeSq << " AND " << LJAvgB1*inv_sizeSq << "\n" << endi;
    }
  } else{
    LJAvgA1 = LJAvgB1 = LJAvgA2 = LJAvgB2 = 0;
    ...
    if ( ! CkMyPe() ) {
      ...
      iout << iINFO << "LONG-RANGE LJ: APPLYING ANALYTICAL CORRECTIONS TO "
           << "ENERGY AND PRESSURE\n" << endi;
      iout << iINFO << "LONG-RANGE LJ: AVERAGE A AND B COEFFICIENTS "
           << LJAvgA*inv_sizeSq << " AND " << LJAvgB*inv_sizeSq << "\n" << endi;
    }
  }
...
// new parm7 reader
Molecule::Molecule(SimParameters *simParams, Parameters *param, AmberParm7Reader::Ambertoppar *amber_data) {
  initialize(simParams, param);
...
void Molecule::read_parm(AmberParm7Reader::Ambertoppar *amber_data) {
#ifdef MEM_OPT_VERSION
  ...
  // Haochuan: It sounds weird that we can generate a compressed PSF from
  // AMBER parameters. Although I read the wiki page in:
  // http://www.ks.uiuc.edu/Research/namd/wiki/index.cgi?NamdMemoryReduction,
  // I still cannot figure out why a user will use this feature for AMBER
  // format.
  if(simParams->genCompressedPsf) {
    atomSegResids = new AtomSegResInfo[numAtoms];
  }
  ...
  for (i = 0; i < numAtoms; ++i) {
    const int resname_length = amber_data->ResNames[amber_data->AtomRes[i]].size();
    const int atomname_length = amber_data->AtomNames[i].size();
    const int atomtype_length = amber_data->AtomSym[i].size();
    atomNames[i].resname = nameArena->getNewArray(resname_length+1);
    atomNames[i].atomname = nameArena->getNewArray(atomname_length+1);
    atomNames[i].atomtype = nameArena->getNewArray(atomtype_length+1);
    if (atomNames[i].resname == NULL || atomNames[i].atomname == NULL || atomNames[i].atomtype == NULL)
      NAMD_die("memory allocation failed when reading atom information");
    // copy data from Ambertoppar to NAMD structures
    strncpy(atomNames[i].resname, amber_data->ResNames[amber_data->AtomRes[i]].c_str(), resname_length+1);
    strncpy(atomNames[i].atomname, amber_data->AtomNames[i].c_str(), atomname_length+1);
    strncpy(atomNames[i].atomtype, amber_data->AtomSym[i].c_str(), atomtype_length+1);
    ...
    // From https://ambermd.org/FileFormats.php:
    /* Amber internally uses units of charge such
     ...
     *
     * Haochuan:
     *   I just copied the documentation from AMBER website and the code from
     *   void Molecule::read_parm(Ambertoppar *amber_data). Here's my derivation
     *   of the magical number 18.2223:
     *       "E = q1*q2/r is in kcal/mol" and "r is in Angstrom"
     *   ==> q1*q2 is in kcal*Å/mol.
     *   ==> q1*q2 is in 4184 Joule*Å/mol.
     *   ==> q1*q2 is in 4184/Na Joule*Å, where Na is the Avogadro constant (6.02214076*1e23 mol^−1)
     *   ==> q1*q2 is in 6.947695e-21 Joule*Å
     *   ==> q1*q2 is in 6.947695e-31 Joule*meter
     *       The charge of a single electron in electron charge units is
     *   ==> qe = 1 e = 1.602176634e-19 Coulomb
     *   ==> qe*qe = (1.602176634e-19)^2 Coulomb*Coulomb
     *             = 2.56697e-38 Coulomb*Coulomb
     *       Multiply qe*qe with the Coulomb constant, 8.9875517923e9 N*meter^2/Coulomb^2
     *   ==> ke*qe*qe = 2.307078e-28 N*meter^2 = 2.307078e-28 Joule*meter
     *   So it turns out if we use AMBER unit, q1*q2 is in 6.947695e-31 Joule*meter.
     *   Let's assume the charge value in AMBER format is x, and the one in electron charge
     *   unit that NAMD uses is y, we have the following equation:
     *       x * x * 6.947695e-31 = y * y * 2.307078e-28,
     *   and then in the following code we need to read x from AMBER format and convert it to y.
     *   The solution is y = x / sqrt(2.307078e-28/6.947695e-31) = x / 18.22262
     *   The difference of 18.22262 and 18.2223 may be due to the changes in constants.
     */
    ...
    if ( simParams->ignoreMass ) {
      ...
      // Haochuan: Does AMBER format allow drude atoms?
      // Drude is a concept in CHARMM ff, since we only care AMBER ff in 
      // AMBER format. Why do we have drude?
      atoms[i].status |= DrudeAtom;
      ++numDrudeAtoms;
    } else if (atoms[i].mass <=3.5) {
      atoms[i].status |= HydrogenAtom;
...
// note that NAMD indexes arrays from 0 to NumAtoms-1.
  ...
  if (amber_data -> HasCMAP) {
    numCrossterms = amber_data->CMAPTermCount;
    if (numCrossterms > 0) {
      crossterms = new Crossterm[numCrossterms];
    }
    for (i = 0; i < numCrossterms; ++i) {
      // phi angle
      crossterms[i].atom1 = amber_data->CMAPIndex[6*i+0]-1;
      crossterms[i].atom2 = amber_data->CMAPIndex[6*i+1]-1;
      crossterms[i].atom3 = amber_data->CMAPIndex[6*i+2]-1;
      crossterms[i].atom4 = amber_data->CMAPIndex[6*i+3]-1;
      // psi angle (has 3 overlapping atoms with phi angle)
      crossterms[i].atom5 = amber_data->CMAPIndex[6*i+1]-1;
      crossterms[i].atom6 = amber_data->CMAPIndex[6*i+2]-1;
      crossterms[i].atom7 = amber_data->CMAPIndex[6*i+3]-1;
      crossterms[i].atom8 = amber_data->CMAPIndex[6*i+4]-1;
      // type
      crossterms[i].crossterm_type = amber_data->CMAPIndex[6*i+5]-1;
    }
  }

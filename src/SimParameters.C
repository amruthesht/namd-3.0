...
//fepe
  ...
  if (FMAMp % FMAFFTBlock != 0)
   ...
   if (groupRestraintsOn) {
     ...
     if (CUDASOAintegrateMode) {
        if (deviceCUDA->getNumDevice() > 1) {
          NAMD_die("GroupRestraints is not compatible with multi-GPU simulation");
        } else {
    ...
    "             Please use single-node single-GPU build with GPUresident on\n"
...
//             NAMD_die("QM Custom PC Selection is incompatible with QMSwitching!");
    ...
    if (SOAintegrateOn) {
	  ...
	  (tclForcesOn && (TCL_MAJOR_VERSION<=8 && TCL_MINOR_VERSION <6)) ||
        ...
        sprintf(msg,
            ...
            "   water models other than TIP3 and TIP4; lonepairs or Drude.\n"
            "\n" 
	...
	if(fixedAtomsOn) strncat(featuremsg,"  fixedAtoms\n",1023);
        if(gridforceOn) strncat(featuremsg,"  gridforce\n",1023);
        if(mgridforceOn) strncat(featuremsg,"  mgridforce\n",1023);
	...
	if(drudeOn) strncat(featuremsg,"  drude\n",1023);
        if(consForceOn) strncat(featuremsg, "  constantForce\n", 1023);
#ifdef NAMD_TCL	
	...
	strncat(msg,featuremsg,2047);
        ...
        if (CUDASOAintegrateMode) {
          ...
          if(CkNumNodes() > 1){
            ...
            if(constraintsOn){
              NAMD_die("Restraints (constraints on) are not compatible with multi-GPU GPUresident");
            }

            if(eFieldOn){
              NAMD_die("EField is not compatible with multi-GPU GPUresident");
            }

            if(SMDOn){
              NAMD_die("SMD is not compatible with multi-GPU GPUresident");
            }
    ...
    if (CUDASOAintegrateMode) {
      if (bondedCUDA != 255) {
        iout << iWARN << "GPUresident cannot delegate the bonded calculations to CPU (bondedGPU != 255).\n";
        iout << iWARN << "Will reset bondedGPU to 255.\n";
        bondedCUDA = 255;
      }
    }
    ...
    if ( useCUDAdisable ) {
      ...
      if ( accelMDOn && (accelMDdihe || accelMDdual) && (bondedCUDA & (0x0004 | 0x0020)) ) {
    ...
    if ( useDeviceMigration ) { 
      if (constraintsOn || SMDOn || groupRestraintsOn || eFieldOn || fixedAtomsOn ||
          tclForcesOn || colvarsOn) {
        updateAtomMap = TRUE;
      }
    }
    if ( useDeviceMigration ) { 
...
/*! Parse the parameters for group restraints */

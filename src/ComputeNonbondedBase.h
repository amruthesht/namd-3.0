...
//#endif
  ...
  ALCH(
    ...
    //JM: switchfactor is set to inf if the switching flag is off
...
#pragma omp ordered simd monotonic(hu:1)
	...
	if ( ku - k  >  6 ) { 	   
	  ...
	  for ( ; k < ku - 2; k +=2 ) {
	    ...
	    if (r2_01[0] <= plcutoff2) {
	      if (UNLIKELY( atom2_0 >= excl_min && atom2_0 <= excl_max ))
		*(pli++) = j0;
	      else 
		*(plin++) = j0;
	    }
	    ...
	    if (r2_01[1] <= plcutoff2) {
	      if (UNLIKELY( atom2_1 >= excl_min && atom2_1 <= excl_max ))
		*(pli++) = j1;
	      else 
		*(plin++) = j1;
	    }
...
#pragma omp ordered simd monotonic(plin:1, pli:1)
	  if (r2 <= plcutoff2) {

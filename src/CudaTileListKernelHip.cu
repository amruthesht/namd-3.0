...
#define DEFAULTKERNEL_NUM_THREAD 256
#define UPDATEPATCHESKERNEL_NUM_THREAD 256
#define CALCPATCHNUMLISTSKERNEL_NUM_THREAD 256
#define BOUNDINGBOXKERNEL_NUM_WARP 4
#define __ldg *
...
//
// Calculate bounding boxes for sets of WARPSIZE atoms
//
...
buildBoundingBoxesKernel(const int atomStorageSize, const float4* __restrict__ xyzq,
  ...
  const int warpId = threadIdx.x / BOUNDINGBOXSIZE;
  const int wid = threadIdx.x % BOUNDINGBOXSIZE;

  // Loop with warp-aligned index to avoid warp-divergence
  for (int iwarp = warpId*BOUNDINGBOXSIZE + blockIdx.x*blockDim.x;iwarp < atomStorageSize;iwarp += blockDim.x*gridDim.x) {
    // Full atom index
    const int i = iwarp + wid;
    // Bounding box index
    const int ibb = i/BOUNDINGBOXSIZE;

    float4 xyzq_i = xyzq[min(atomStorageSize-1, i)];

    volatile float3 minxyz, maxxyz;

    typedef cub::BlockReduce<float, BOUNDINGBOXSIZE> BlockReduce;
    __shared__ typename BlockReduce::TempStorage tempStorage[BOUNDINGBOXKERNEL_NUM_WARP];
    minxyz.x = BlockReduce(tempStorage[warpId]).Reduce(xyzq_i.x, cub::Min());
    minxyz.y = BlockReduce(tempStorage[warpId]).Reduce(xyzq_i.y, cub::Min());
    minxyz.z = BlockReduce(tempStorage[warpId]).Reduce(xyzq_i.z, cub::Min());
    maxxyz.x = BlockReduce(tempStorage[warpId]).Reduce(xyzq_i.x, cub::Max());
    maxxyz.y = BlockReduce(tempStorage[warpId]).Reduce(xyzq_i.y, cub::Max());
    maxxyz.z = BlockReduce(tempStorage[warpId]).Reduce(xyzq_i.z, cub::Max());
...
__global__ void calcTileListPosKernel(const int numComputes,
  ...
  for (int base=0;base < numComputes;base+=nthread) {
    ...
    int numTiles1 = (k < numComputes) ? (patches[computes[k].patchInd.x].numAtoms-1)/BOUNDINGBOXSIZE+1 : 0;
...
__global__ void updatePatchesKernel(const int numComputes,
  ...
  for (int k = (threadIdx.x + blockIdx.x*blockDim.x)/nthread;k < numComputes;k+=blockDim.x*gridDim.x/nthread)
  {
    ...
    int numTiles1 = (patches[patchInd.x].numAtoms-1)/BOUNDINGBOXSIZE+1;
...
buildTileListsBBKernel(const int numTileLists,
  ...
  HIP_DYNAMIC_SHARED( char, sh_buffer)
  ...
  // Loop with warp-aligned index to avoid warp-divergence
  for (int iwarp = (threadIdx.x/BOUNDINGBOXSIZE)*BOUNDINGBOXSIZE + blockIdx.x*blockDim.x;iwarp < numTileLists;iwarp += blockDim.x*gridDim.x) {
    ...
    if (itileList < numTileLists) {
      ...
      numTiles2 = (patch2.numAtoms-1)/BOUNDINGBOXSIZE+1;
      int tileStart1 = patch1.atomStart/BOUNDINGBOXSIZE;
      int tileStart2 = patch2.atomStart/BOUNDINGBOXSIZE;
    ...
    typedef cub::BlockScan<int, BOUNDINGBOXSIZE> BlockScan;
    __shared__ typename BlockScan::TempStorage tempStorage;
    int active = (itileListLen > 0);
    int activePos;
    BlockScan(tempStorage).ExclusiveSum(active, activePos);
    int itileListPos;
    BlockScan(tempStorage).ExclusiveSum(itileListLen, itileListPos);
    int jtileStart, numJtiles;
    // Last thread in the warp knows the total number
    if (wid == BOUNDINGBOXSIZE-1) {
    ...
    numJtiles  = cub::ShuffleIndex<BOUNDINGBOXSIZE>(numJtiles,  BOUNDINGBOXSIZE-1, WARP_FULL_MASK);
    jtileStart = cub::ShuffleIndex<BOUNDINGBOXSIZE>(jtileStart, BOUNDINGBOXSIZE-1, WARP_FULL_MASK);
    ...
    int jEnd   = cub::ShuffleDown<BOUNDINGBOXSIZE>(itileListPos, 1, BOUNDINGBOXSIZE-1, WARP_FULL_MASK);    
    if (wid == BOUNDINGBOXSIZE-1) jEnd = numJtiles;

    if (itileListLen > 0) {
      // Setup tileLists[]
      //TileList TLtmp;
      tileLists[itileList].iatomStart = patch1.atomStart + i*BOUNDINGBOXSIZE;
      tileLists[itileList].jtileStart = jtileStart + jStart;
      tileLists[itileList].jtileEnd   = jtileStart + jEnd - 1;
      tileLists[itileList].patchInd   = patchInd;
      tileLists[itileList].offsetXYZ  = offsetXYZ;
      tileLists[itileList].icompute   = icompute;
      ...
      for (int j=0;j < numTiles2;j++) {
        if (sh_tile[j]) {
          tileJatomStart[jtile] = patch2.atomStart + j*BOUNDINGBOXSIZE;
          jtile++;
        }
      }
...
#define REPACKTILELISTSKERNEL_NUM_WARP 1
__global__ void
repackTileListsKernel(const int numTileLists, const int begin_bit, const int* __restrict__ tileListPos,
  ...
  for (int i = threadIdx.x/BOUNDINGBOXSIZE + blockDim.x/BOUNDINGBOXSIZE*blockIdx.x;i < numTileLists;i+=blockDim.x/BOUNDINGBOXSIZE*gridDim.x)
  {
    ...
    if (wid == 0) {
      // TileList tileList;
      tileListsDst[i].iatomStart = iatomStart;
      tileListsDst[i].offsetXYZ  = offsetXYZ;
      tileListsDst[i].jtileStart = start;
      tileListsDst[i].jtileEnd   = end;
      tileListsDst[i].patchInd   = patchInd;
      tileListsDst[i].icompute   = icompute;
    ...
    if (jtiles == NULL) {
      ...
      int jtile = start;
      for (int jtileOld=startOld;jtileOld <= endOld;jtileOld+=BOUNDINGBOXSIZE,jtile+=BOUNDINGBOXSIZE) {
        if (jtileOld + wid <= endOld) {
          // this changes the values of tileListdst it seems. why?
      ...
      int jtile0 = start;
      for (int jtileOld=startOld;jtileOld <= endOld;jtileOld+=BOUNDINGBOXSIZE) {
        int t = jtileOld + wid;
        int jtile = (t <= endOld) ? jtiles[t] : 0;
        jtile >>= begin_bit;
        jtile &= 65535;
        typedef cub::BlockScan<int, BOUNDINGBOXSIZE> BlockScan;
        __shared__ typename BlockScan::TempStorage tempStorage[REPACKTILELISTSKERNEL_NUM_WARP];
        int warpId = threadIdx.x / BOUNDINGBOXSIZE;
        int jtilePos;
        BlockScan(tempStorage[warpId]).ExclusiveSum(jtile, jtilePos);

        if (jtile) tileJatomStartDst[jtile0+jtilePos] = __ldg(&tileJatomStartSrc[t]);

        WarpMask b = NAMD_WARP_BALLOT(WARP_FULL_MASK, jtile);
        if (tileExclsSrc != NULL) {
          while (b != 0) {
...
//
#define SORTTILELISTSKERNEL_NUM_THREAD 256
#define SORTTILELISTSKERNEL_ITEMS_PER_THREAD 15
...
CudaTileListKernel::~CudaTileListKernel() {
  ...
  if (part != NULL)  deallocate_device<char>(&part);
...
void CudaTileListKernel::writeTileList(FILE* handle, const int numTileLists,
  const TileList* d_tileLists, cudaStream_t stream) {
...
void CudaTileListKernel::writeTileJatomStart(FILE* handle, const int numJtiles,
  const int* d_tileJatomStart, cudaStream_t stream) {
...
void CudaTileListKernel::writeTileExcls(FILE* handle, const int numJTiles, 
  const TileExcl* d_tileExcl, cudaStream_t stream){
  TileExcl* h_tileExcl = new TileExcl[numJtiles];
  copy_DtoH<TileExcl>(d_tileExcl, h_tileExcl, numJtiles, stream);
  cudaCheck(cudaStreamSynchronize(stream));
  for (int i=0;i < numJtiles;i++) {
    for(int j = 0; j < WARPSIZE; j++) fprintf(handle, " %u ", h_tileExcl[i].excl[j]);
    fprintf(handle, "\n");
  }
  delete [] h_tileExcl;
}
...
void CudaTileListKernel::prepareBuffers(
  ...
  reallocate_device<float4>(&xyzq, &xyzqSize, atomStorageSizeIn, OVERALLOC);

  // Copy cudaPatches to device
  reallocate_device<CudaPatchRecord>(&cudaPatches, &cudaPatchesSize, numPatchesIn);
  copy_HtoD<CudaPatchRecord>(h_cudaPatches, cudaPatches, numPatchesIn, stream);
}
...
void CudaTileListKernel::buildTileLists(const int numTileListsPrev,
  ...
  cudaStream_t stream, const bool atomsChanged, const bool allocatePart, 
  bool CUDASOAintegratorOn, bool deviceMigration) {
  ...
  // Copy cudaPatches to device
  // If DeviceMigration is on then this happens in prepareBuffers()
  if (!CUDASOAintegratorOn || !deviceMigration) {
    reallocate_device<CudaPatchRecord>(&cudaPatches, &cudaPatchesSize, numPatches);
    copy_HtoD<CudaPatchRecord>(h_cudaPatches, cudaPatches, numPatches, stream);
  }
  ...
  // Calculate tile list positions (tilePos)
  {
    ...
    int nblock = 1;
    calcTileListPosKernel<DEFAULTKERNEL_NUM_THREAD> <<< nblock, nthread, 0, stream >>> (
    numComputes, cudaComputes, cudaPatches, tilePos);
    cudaStreamSynchronize(stream);// for now
  ...
  // Build (tileLists1.patchInd, tileLists1.offsetXYZ)
  {
    ...
    int nblock = min(deviceCUDA->getMaxNumBlocks(), (numComputes-1)/(nthread/BOUNDINGBOXSIZE)+1);
    updatePatchesKernel<BOUNDINGBOXSIZE> <<< nblock, BOUNDINGBOXSIZE, 0, stream >>> (
  ...
  if (!deviceMigration) {
    if(atomsChanged) reallocate_device<float4>(&xyzq, &xyzqSize, atomStorageSize, OVERALLOC);
    if(!CUDASOAintegratorOn || atomsChanged) copy_HtoD<float4>(h_xyzq, xyzq, atomStorageSize, stream);
  }
  
  //This should be allocated for FEP only
  if(allocatePart) reallocate_device<char>(&part, &partSize, atomStorageSize, OVERALLOC);
  
  // Fills in boundingBoxes[0 ... numBoundingBoxes-1]
  {
    int numBoundingBoxes = atomStorageSize/BOUNDINGBOXSIZE;
    ...
    // int nthread = BOUNDINGBOXSIZE*nwarp;
    int nthread = BOUNDINGBOXSIZE*1;  // Changing this value to 1 as we want to use BlockReduce primitives
...

  {
    ...
    // int nthread = BOUNDINGBOXSIZE*nwarp;
    int nthread = BOUNDINGBOXSIZE*1; // 1 for now to use BlockReduce.
...
#if 0
  writeTileList("tileListBefore_HIP.txt", numTileLists, tileLists1, stream);
  writeTileJatomStart("tileJatomStartBefore_HIP.txt", numJtiles, tileJatomStart1, stream);
#endif
...
#if 0
  writeTileList("tileListAfter_HIP.txt", numTileLists, tileLists1, stream);
  writeTileJatomStart("tileJatomStartAfter_HIP.txt", numJtiles, tileJatomStart1, stream);
#endif
...
void CudaTileListKernel::sortTileLists(
  ...
  if (doStreaming)
  {
    ...
    if (doOutputOrder && useJtiles) {
      ...
      // Calculate position from depth
      {
        ...
        {
          size_t size = 0;
          cudaCheck((cudaError_t)cub::DeviceScan::ExclusiveSum(NULL, size,
          ...
          cudaCheck((cudaError_t)cub::DeviceScan::ExclusiveSum((void *)tempStorage, size,
            (int *)tileListDepthDst.ptr, tileListPos, numTileListsSrc, stream));
        }
      }
      ...
      else
      {
        ...
        size_t size = 0;
        cudaCheck((cudaError_t)cub::DeviceRadixSort::SortPairs(NULL, size,
        ...
        reallocate_device<char>(&tempStorage, &tempStorageSize, size, 1.5f);
        size = tempStorageSize;
        cudaCheck((cudaError_t)cub::DeviceRadixSort::SortPairs((void *)tempStorage, size,
          sortKeySrc, sortKeyDst, tileListOrderSrc.ptr, tileListOrderDst.ptr,
          numTileListsSrc, 0, endbit_tmp, stream));
      }
      ...
      // Local sort
      {
        int nblock = min(deviceCUDA->getMaxNumBlocks(), (numTileListsDst-1)/WARPSIZE+1);
        localSort<WARPSIZE> <<< nblock, WARPSIZE, 0, stream >>> (
  ...
  else
  {
    ...
    else
    {
      // Long list, sort on multiple thread blocks
      size_t size = 0;
      cudaCheck((cudaError_t)cub::DeviceRadixSort::SortPairsDescending(NULL, size,
      ...
      reallocate_device<char>(&tempStorage, &tempStorageSize, size, 1.5f);
      size = tempStorageSize;
      cudaCheck((cudaError_t)cub::DeviceRadixSort::SortPairsDescending((void *)tempStorage, size,
        tileListDepthSrc.ptr, tileListDepthDst.ptr, tileListOrderSrc.ptr, tileListOrderDst.ptr,
        numTileListsSrc, begin_bit, end_bit, stream));
    }
  }
  ...
  // --------------------------------------------------------------------
  {
    ...
    int nthread = BOUNDINGBOXSIZE*REPACKTILELISTSKERNEL_NUM_WARP;
  ...
  if (doStreaming)
  {
    ...
    int nthread = CALCPATCHNUMLISTSKERNEL_NUM_THREAD;
    int nblock = min(deviceCUDA->getMaxNumBlocks(), (numTileListsDst-1)/nthread+1);
...
//
void CudaTileListKernel::reSortTileLists(const bool doGBIS, const bool CUDASOAIntegratorOn, cudaStream_t stream) {

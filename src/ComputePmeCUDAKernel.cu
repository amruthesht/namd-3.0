...
#define WARP (threadIdx.x>>6)
#define THREAD (threadIdx.x&63)
#define FX bspline_factors[threadIdx.x>>6][0]
#define FY bspline_factors[threadIdx.x>>6][1]
#define FZ bspline_factors[threadIdx.x>>6][2]
#define DFX bspline_dfactors[threadIdx.x>>6][0]
#define DFY bspline_dfactors[threadIdx.x>>6][1]
#define DFZ bspline_dfactors[threadIdx.x>>6][2]

#define FX2 bspline_2factors[threadIdx.x>>6][0]
#define FY2 bspline_2factors[threadIdx.x>>6][1]
#define FZ2 bspline_2factors[threadIdx.x>>6][2]

#define ATOM atoms[threadIdx.x>>6].a2d[i_atom]
#define AX atoms[threadIdx.x>>6].a2d[i_atom][0]
#define AY atoms[threadIdx.x>>6].a2d[i_atom][1]
#define AZ atoms[threadIdx.x>>6].a2d[i_atom][2]
#define AQ atoms[threadIdx.x>>6].a2d[i_atom][3]
#define AI atoms[threadIdx.x>>6].a2d[i_atom][4]
#define AJ atoms[threadIdx.x>>6].a2d[i_atom][5]
#define AK atoms[threadIdx.x>>6].a2d[i_atom][6]
...
__global__ void cuda_pme_charges_dev(
  ...
  for ( int i_atom=0; i_atom < atoms_this_warp; ++i_atom ) {
#if defined(NAMD_HIP)
    NAMD_WARP_SYNC(WARP_FULL_MASK);  // done writing atoms, reading bspline_factors (FX,FY,FZ)
#else
    WARP_SYNC(WARP_FULL_MASK); 
#endif
    float aq=AQ;
...
#if defined(NAMD_HIP)
    // NAMD_WARP_SYNC(WARP_FULL_MASK);  // done writing atoms
    __syncthreads();
#else
...
#if defined(NAMD_HIP)
    // NAMD_WARP_SYNC(WARP_FULL_MASK);  // done writing force_reduction
    __syncthreads();
#else
...
#ifdef NAMD_HIP
                                         + force_reduction[WARP].a1d[THREAD + 96]
                                         + force_reduction[WARP].a1d[THREAD + 120]
                                         + force_reduction[WARP].a1d[THREAD + 144]
                                         + force_reduction[WARP].a1d[THREAD + 168]
#endif
                                         ;
    }
#if defined(NAMD_HIP)

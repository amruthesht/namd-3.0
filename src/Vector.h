...
class Vector {
   public:
     ...
     #ifndef NAMD_AVXTILES
     NAMD_HOST_DEVICE Vector(void) : x(-99999), y(-99999), z(-99999) { ; }
     #else
     ...
     NAMD_HOST_DEVICE Vector( BigReal newv )  // allow Vector v = 0; etc.
       : x(newv), y(newv), z(newv) { ; }

     NAMD_HOST_DEVICE Vector(const FloatVector &v) : x(v.x), y(v.y), z(v.z) { ; }


#if defined(NAMD_CUDA) || defined(NAMD_HIP)
     // Allows for implicit conversion from double3
     NAMD_HOST_DEVICE Vector(double3 vec)
       : x(vec.x), y(vec.y), z(vec.z) { ; }

     // Allows for implicit conversion to double3
     ...
     // NAMD_die is not supported on device
...
//         NAMD_die("Division by 0 on a vector operation.");
     ...
     NAMD_HOST_DEVICE BigReal rlength (void) {
     ...
     // returns (*this) * V2
     NAMD_HOST_DEVICE BigReal dot(const Vector &v2) {

...
__global__ void calcTileListPosKernel(const int numComputes,
  ...
  for (int base=0;base < numComputes;base+=nthread) {
    ...
    int numTiles1 = (k < numComputes) ?
        (CudaComputeNonbondedKernel::computeNumTiles(patches[computes[k].patchInd.x].numAtoms, WARPSIZE)) : 0;
...
__global__ void updatePatchesKernel(const int numComputes,
  ...
  for (int k = (threadIdx.x + blockIdx.x*blockDim.x)/nthread;k < numComputes;k+=blockDim.x*gridDim.x/nthread)
  {
    ...
    int numTiles1 = CudaComputeNonbondedKernel::computeNumTiles(patches[patchInd.x].numAtoms);
...
buildTileListsBBKernel(const int numTileLists,
  ...
  for (int iwarp = (threadIdx.x/WARPSIZE)*WARPSIZE + blockIdx.x*blockDim.x;iwarp < numTileLists;iwarp += blockDim.x*gridDim.x) {
    ...
    if (itileList < numTileLists) {
      ...
      numTiles2 = CudaComputeNonbondedKernel::computeNumTiles(patch2.numAtoms);
      ...
      for (int jtileOld=startOld;jtileOld <= endOld;jtileOld+=WARPSIZE) {
        ...
        if (tileExclsSrc != NULL) {
          ...
          while (b != 0) {
            ...
            // Promote k to 64-bit integer. DMC doesn't think this is necessary
            // but it resolves issues for CUDA 11.6
#if 1
            int64_t k = __ffs(b) - 1;
#else
            int k = __ffs(b) - 1;
...
void CudaTileListKernel::buildTileLists(const int numTileListsPrev,
  ...
  bool CUDASOAintegrator, bool deviceMigration) {
  ...
  if (!CUDASOAintegrator || !deviceMigration) {
  ...
  // deviceMigration could be true with CUDASOAintegrator as false if minimization is being
  // performed. So we want this to be true if CUDASOAintegrator is false or if deviceMigration
  // is false
  if (!CUDASOAintegrator || !deviceMigration) {
    if(atomsChanged) reallocate_device<float4>(&xyzq, &xyzqSize, atomStorageSize, OVERALLOC);
    if(!CUDASOAintegrator || atomsChanged) copy_HtoD<float4>(h_xyzq, xyzq, atomStorageSize, stream);
  }

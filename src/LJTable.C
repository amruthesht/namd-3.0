...
//  printf("Looking at interaction of  %i with %i\n", i, j);
  if ( tabulatedEnergies && params->get_table_pair_params(i_type, j_type ,&K)) {
//    printf("Making this interaction tabulated. %i %i %i\n", i, j, K);
  ...
  else if (params->get_vdw_pair_params(i_type, j_type, &A, &B, &A14, &B14))
  {
    cur->A = A;
    cur->B = B;
    cur_scaled->A = A14;
  ...
  if (soluteScalingOn) {
    const BigReal soluteScalingFactor = simParams->soluteScalingFactorVdw;

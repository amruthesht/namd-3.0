...
void Patch::positionsReady(int doneMigration, int startup)
{
   DebugM(4,"Patch::positionsReady() - patchID(" << patchID <<")"<<std::endl );
   SimParameters *simParams = Node::Object()->simParameters;

   // The atom map needs to be updated for host migration, device migration startup and 
   // device migration + some advanced features
   const bool updateAtomMap = (startup || 
                               !simParams->useDeviceMigration || 
                               simParams->updateAtomMap);
   if (doneMigration && updateAtomMap) {
// #ifdef REMOVE_PROXYDATAMSG_EXTRACOPY
...
//#if 0
   // Clear forces if GPU-offload or GPU-resident without device migration
   const bool clearForces = (!simParams->CUDASOAintegrate || 
                             (doneMigration && !simParams->useDeviceMigration) );
   for ( int j = 0; j < Results::maxNumForces; ++j )
   {
     f[j].resize(numAtoms);
     forcePtr = f[j].begin();
     if(clearForces) memset (forcePtr, 0, sizeof (Force) * numAtoms);
    ...
    Force *pf = results.f[1];
    int k=0;
    printf("%s, line %d\n", __FILE__, __LINE__);
    printf("  initial:  pf[%d] = %f %f %f\n", k, pf[k].x, pf[k].y, pf[k].z);
#endif
#endif
   // fprintf(stderr, "(Pe[%d] tstep %d) opening box on patch %d\n", 
   ...
   if ( ! computesSortedByPriority ) {
     ...
     // XXX TODO: Maybe get rid of this sorting and see what happens

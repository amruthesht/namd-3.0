                        ...
                        if (echoWarn)
                        {
                          ...
                          if (!replace)
                          {
                            ...
                            if (ptr->multiplicity > MAX_MULTIPLICITY)
                            {
                              ...
                              snprintf(err_msg, sizeof(err_msg),
                                  "Multiple dihedral with multiplicity "
                                  "of %d greater than max of %d",
...

{
  ...
  if ( epsilon < 0. || epsilon14 < 0. ) {
    if (paramType == paraCharmm) {
      iout << iWARN << "Ignoring VDW parameter with positive epsilon:\n"
          << buf << "\n" << endi;
    }
    else {
...
void Parameters::assign_bond_index(const char *atom1, const char *atom2, Bond *bond_ptr, bool* bond_found /* = nullptr */)
...

{
 ...
 if (paramType == paraXplor) {
  ...
  if (multiplicity > maxDihedralMults[ptr->index])
  {
    ...
    snprintf(err_msg, sizeof(err_msg),
        "Multiplicity of Paramters for diehedral bond %s %s %s %s "
        "of %d exceeded", 
        atom1, atom2, atom3, atom4, maxDihedralMults[ptr->index]);
    NAMD_die(err_msg);
  }
 ...
 if (paramType == paraXplor) {
  ...
  if (multiplicity > maxImproperMults[ptr->index])
  {
    ...
    snprintf(err_msg, sizeof(err_msg),
        ...
        "of %d exceeded", 
        atom1, atom2, atom3, atom4, maxImproperMults[ptr->index]);
    NAMD_die(err_msg);
  }
...
// new AMBER parm7 reader
Parameters::Parameters(AmberParm7Reader::Ambertoppar *amber_data, BigReal vdw14)
{
  initialize();
...
void Parameters::read_parm(AmberParm7Reader::Ambertoppar *amber_data, BigReal vdw14)
{
  int i,j,ico,numtype,mul;
  IndexedVdwPair *new_node;

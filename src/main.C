...
#error "Charm++ 6.5.1 or later is required to build NAMD"
    ...
    iout << iINFO << "Using bitfields in atom data structures.\n" << endi;
#else
    iout << iINFO << "Using NO bitfields in atom data structures.\n" << endi;
#endif
    iout << iINFO << "  sizeof( CompAtom )    = " << int(sizeof(CompAtom))
         << "\n"
         << iINFO << "  sizeof( CompAtomExt ) = " << int(sizeof(CompAtomExt))
         << "\n"
         << endi;
#endif
  }

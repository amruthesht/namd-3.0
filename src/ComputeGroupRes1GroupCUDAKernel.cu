...
/*! Compute restraint force, virial, and energy applied to large
    ...
    (h_group2COM) to a reference point (h_group1COMRef). 
    To use this function, the COM of the group 2 must be calculated 
    and passed to this function as h_group2COM. 
    This function also calculates the distance from ref point to
    COM of the group 2. */
template<int T_DOENERGY, int T_DOVIRIAL, int T_USEMAGNITUDE>
__global__ void computeLargeGroupRestraintKernel_1Group( 
    ...
    const double3*  __restrict    h_group1COMRef,
    ...
    unsigned int*   __restrict    d_tbcatomic)
{
    ...
    if(tIdx < numRestrainedGroup) {
        SOAindex = groupAtomsSOAIndex[tIdx];
        m = mass[SOAindex];
        // Calculate distance from ref to com2 along specific restraint dimention 
        diffCOM.x = (h_group2COM->x - h_group1COMRef->x) * resDirection.x;
        diffCOM.y = (h_group2COM->y - h_group1COMRef->y) * resDirection.y;
        diffCOM.z = (h_group2COM->z - h_group1COMRef->z) * resDirection.z;
    ...
    if(T_DOENERGY || T_DOVIRIAL) {
        ...
        // Thread 0 of the last block will set the host values
            if(threadIdx.x == 0) {
                if(T_DOENERGY) {
                    h_resEnergy[0] = energy;     // restraint energy
                    h_diffCOM->x = diffCOM.x;    // distance from ref position to COM of group 2 
                    h_diffCOM->y = diffCOM.y;    // distance from ref position to COM of group 2  
                    h_diffCOM->z = diffCOM.z;    // distance from ref position to COM of group 2 
                ...
                if(T_DOVIRIAL) { 
                    ...
                    //reset the device virial values
                    d_virial->xx = 0;
                    d_virial->xy = 0;
                    d_virial->xz = 0;
...
/*! Compute restraint force, virial, and energy applied to small
    ...
    COM of the group 2. */
template<int T_DOENERGY, int T_DOVIRIAL, int T_USEMAGNITUDE>
__global__ void computeSmallGroupRestraintKernel_1Group(
    const int         numRestrainedGroup,
    const int         restraintExp,
    const double      restraintK,
    const double3     resCenterVec,
    ...
    double3*        __restrict    h_diffCOM)
{
    ...
    // reduce the (mass * position) values for the thread block
    ...
    // Thread 0 calculates the COM
    if(threadIdx.x == 0){
        sh_com2.x = com2.x * inv_group2_mass; // calculates the current center of mass
        sh_com2.y = com2.y * inv_group2_mass; // calculates the current center of mass
        sh_com2.z = com2.z * inv_group2_mass; // calculates the current center of mass
    }
    __syncthreads();

    if(tIdx < numRestrainedGroup){
        // calculate the distance from ref to com2 along specific restraint dimention
        diffCOM.x = (sh_com2.x - h_group1COMRef->x) * resDirection.x;
        diffCOM.y = (sh_com2.y - h_group1COMRef->y) * resDirection.y; 
        diffCOM.z = (sh_com2.z - h_group1COMRef->z) * resDirection.z;
        // Calculate the minimum image distance 
        diffCOM = lat.delta_from_diff(diffCOM);
    ...
    cudaStream_t      stream)
{
    ...
    if (numRestrainedGroup > 1024) {
        const int blocks = 128; 
        const int grid = (numRestrainedGroup + blocks - 1) / blocks;
        //first calculate the COM for restraint group and store it in h_group2COM 
        ...
        computeLargeGroupRestraintKernel_1Group<DOENERGY, DOVIRIAL, USEMAGNITUDE>\
        <<<grid, blocks, 0, stream>>>( \
            numRestrainedGroup, restraintExp, restraintK, resCenterVec, \
            resDirection, inv_group2_mass, d_groupAtomsSOAIndex, lat, \
            d_transform, d_mass, d_pos_x, d_pos_y, d_pos_z, \
            d_f_normal_x, d_f_normal_y, d_f_normal_z,  d_virial, \
            h_extVirial, h_resEnergy, h_resForce, h_group1COMRef, \
        ...
        computeSmallGroupRestraintKernel_1Group<DOENERGY, DOVIRIAL, USEMAGNITUDE>\
        <<<grid, blocks, 0, stream>>>( \
            numRestrainedGroup, restraintExp, restraintK, resCenterVec, \
            resDirection, inv_group2_mass, d_groupAtomsSOAIndex, lat, \
            d_transform, d_mass, d_pos_x, d_pos_y, d_pos_z, \
            d_f_normal_x, d_f_normal_y, d_f_normal_z, \
            h_extVirial, h_resEnergy, h_resForce, \
            h_group1COMRef, h_diffCOM);

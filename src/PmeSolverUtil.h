...
protected:
  ...
  int j0, k0;
  // indicate the grid index in multiple grids
  unsigned int multipleGridIndex;
public:
  PmeKSpaceCompute(PmeGrid pmeGrid, const int permutation,
    const int jblock, const int kblock, double kappa, unsigned int multipleGridIndex = 0) : 
    pmeGrid(pmeGrid), permutation(permutation),
    jblock(jblock), kblock(kblock), kappa(kappa), multipleGridIndex(multipleGridIndex) {
...
protected:
  ...
  // Pencil position 
  const int jblock, kblock;
  // Grid index in multiple grids
  unsigned int multipleGridIndex;
public:
  PmeRealSpaceCompute(PmeGrid pmeGrid, const int jblock, const int kblock, unsigned int grid = 0) : 
    pmeGrid(pmeGrid), jblock(jblock), kblock(kblock), data(NULL), multipleGridIndex(grid) {
      int x0, x1, y1, z1;
...
// WORKAROUND-HIP: vector types are "using", not struct, and cannot be forward-declared
#ifdef NAMD_HIP

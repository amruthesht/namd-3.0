...
/// Set sizes based on number of atoms and padding factor.
/// Sets internally numAtoms, maxAtoms, and numBytes.
/// Returns numBytes.
///
...
/// Caller provides the buffer space to be used.
/// Set all pointers relative to this provided
/// address, pointing into the buffer space.
///
...
//  void mollify(Vector qtilde[],const Vector q0[],const BigReal lambda[], Vector force[],const int n, const int m, const BigReal imass[],const int ial[],const int ibl[],const Vector refab[]); 
  ...
  ScaledPosition getMin() {return min;}
  ScaledPosition getMax() {return max;}
...
private:
  ...
  FullAtomList sortatom;    ///< for moving solvent to end of atom array
...
#if defined(NAMD_CUDA) || defined(NAMD_HIP)
  unsigned char* soa_buffer = nullptr;
  size_t soa_buffer_size = 0;
#else
  ResizeArray<unsigned char> soa_buffer;
#endif
  /**< Split out the memory management from PatchDataSOA.
   * Now we can use any memory management approach we want,
   * including those supporting device memory.
   * Want buffer allocation to be appropriately word aligned.
   */
  ...
  /// and save nonbonded and slow forces
  ///
  ...
  /*! Zero SOA global forces before calculating bias forces called by
      computeGlobal. */

  void zero_global_forces_SOA();

  /// Clears the atom map
  /// Used by device migration to clear maps before HomePatch data has been updated
  void clearAtomMap();

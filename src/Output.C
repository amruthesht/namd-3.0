...
#ifdef NODEGROUP_FORCE_REGISTER
    if(namdMyNode->simParams->CUDASOAintegrate){
      CProxy_PatchData cpdata(CkpvAccess(BOCclass_group).patchData);
      PatchData* pData = cpdata.ckLocalBranch();
      pData->pdbData->set_all_positions(coor);
      pData->pdbData->write(restart_name, comment);
    }
    else
#endif
    {
...

{
  ...
  if (!namdMyNode->simParams->binaryRestart)
  {
    ...
    if(namdMyNode->simParams->CUDASOAintegrate){
      CProxy_PatchData cpdata(CkpvAccess(BOCclass_group).patchData);
      PatchData* pData = cpdata.ckLocalBranch();
      pData->pdbData->set_all_positions(vel);
      pData->pdbData->write(restart_name, comment);
    }
    else
#endif
...

{
  ...
  if (!namdMyNode->simParams->binaryOutput)
  {
    ...
    if(namdMyNode->simParams->CUDASOAintegrate){
      ...
      PatchData* pData = cpdata.ckLocalBranch();
      pData->pdbData->set_all_positions(coor);
      pData->pdbData->write(output_name, comment);
    }
    else
#endif
    {
...

{
  ...
  if (!(namdMyNode->simParams->binaryOutput))
  {
    ...
    if(namdMyNode->simParams->CUDASOAintegrate){
      CProxy_PatchData cpdata(CkpvAccess(BOCclass_group).patchData);
      PatchData* pData = cpdata.ckLocalBranch();
      pData->pdbData->set_all_positions(frc);
      pData->pdbData->write(output_name, comment);
    }
    else
#endif
